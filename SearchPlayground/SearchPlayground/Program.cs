...
namespace SearchPlayground
{
    ...
    { 
        ...
        static int index;
        static int l = 0; //low - spodní strana array
        static int u = 0; // up - horní strana array
        static int m = 0; // mid - střed array
        ...
        static int BinarySearch(int[] array, int elementToSearch)
        {
             ...
             */
            Array.Sort(array);
            l = 0; //low - spodní strana array
            u = array.Length - 1; // up - horní strana array
            m = 0; // mid - střed array
            while (l <= u)
            {
                counter++;
        ...
        static void SearchArray(int[] array, string arrayName)
        {
            ...
            Console.WriteLine($"    Lineární vyhledávání našlo prvek {randomElement} na indexu {index} během {counter} kol");
            counter = 0;

            index = BinarySearch(array, randomElement);
            Console.WriteLine($"    Binární vyhleádávání našlo prvek {randomElement} na indexu {index} během {counter} kol");
            counter = 0;

            index = BinarySearchRecursive(array, randomElement, 0, array.Length - 1);
            Console.WriteLine($"    Rekurzivní binární vyhledávání našlo prvek {randomElement} na indexu {index} během {counter} kol");
            counter = 0;
